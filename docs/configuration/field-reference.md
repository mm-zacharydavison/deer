...
| `write_paths` | string[] | `[]` | global | Host paths to grant write access inside the sandbox (e.g. for hooks that write outside the worktree). Paths starting with `~/` are resolved to `$HOME`. |
