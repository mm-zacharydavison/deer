...
export interface UpdateOptions {
  /** Binary prefix, e.g. "deer" or "deerbox" */
  name: string;
  /** Current version string (semver, no "v" prefix) */
  version: string;
}
 ...
 */
export async function checkAndUpdate({ name, version }: UpdateOptions): Promise<boolean> {
  if (isDevMode()) return false;

  // Guard against infinite update loops — if we were just updated, skip the check.
  if (process.env.DEER_UPDATED) {
    delete process.env.DEER_UPDATED;
    return false;
  }
  ...
  if (!isNewer(latestVersion, version)) return false;
  ...
  process.stdout.write(
    `${name}: updating v${version} → v${latestVersion}...\n`
  );
  ...
  try {
    ...
    process.stdout.write(`${name}: update download failed, continuing with current version\n`);
    return false;
  }
  ...
  try {
    ...
    process.stdout.write(`${name}: update install failed, continuing with current version\n`);
    return false;
  }

  process.stdout.write(`${name}: updated to v${latestVersion}, restarting...\n`);
  ...
  const proc = Bun.spawn([currentPath, ...process.argv.slice(1)], {
    ...
    stdout: "inherit",
    stderr: "inherit",
    env: { ...process.env, DEER_UPDATED: "1" },
  });
